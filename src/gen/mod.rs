...
static STDLIB_PATH: LazyLock<OsString> = LazyLock::new(|| "lib/lib.c".into());
...
impl<'a, 'ctx, 'alloc> Emitter<'a, 'ctx, 'alloc> {
    ...
    fn emit_expr(&mut self, expr: &Expr) -> BasicValueEnum<'ctx> {
        match &expr.kind {
            ...
            ExprKind::PrefixOp(Spanned(_, PrefixOpKind::Amp | PrefixOpKind::AmpMut), expr) => {
                self.emit_place_expr(expr).into()
            }
    ...
    fn emit_indexable_ty_parts(
        ...
        let lhs_ptr_value = if num_derefs > 0 {
            ...
            self.emit_place_expr(lhs)
        };

        let lhs_value = self
            .builder
            .build_load(lhs_ptr_value.into_pointer_value(), "");
        ...
        match autoderef_ty {
            Type::Array(ty, size) => (
                self.builder.build_pointer_cast(
                    lhs_ptr_value.into_pointer_value(),
